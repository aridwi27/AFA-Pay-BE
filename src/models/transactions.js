...
module.exports = {
  ...
  mDetailTrans: (id) => {
    return new Promise((resolve, reject) => {
      const sql = `SELECT transactions.id as id, transactions.status as status, transactions.trans_id as trans_id, transactions.user_id as user_id,  user.first_name as userFirstName, user.last_name as userLastName, user.image as userImage, user.handphone as userHandphone, transactions.target_id as target_id, target.first_name as targetFirstName, target.last_name AS targetLastName, target.image as targetImage, target.handphone as targetHandphone, transactions.amount as amount, transactions.creditLeft as creditLeft, transactions.type as type, transactions.info as info, transactions.created_at as created_at, transactions.updated_at as updated_at FROM transactions LEFT JOIN users as user ON transactions.user_id = user.id LEFT JOIN users as target ON transactions.target_id = target.id WHERE transactions.id = ${id}`
        connection.query(sql, (err, result) => {
            if (err) {
                reject(new Error(err));
            } else {
