...
module.exports = {
  mAllTrans: (user, offset, limit, sort, range) => {
    return new Promise((resolve, reject) => {
      let sql = `SELECT transactions.id as id, user.first_name as userFirstName, user.last_name as userLastName, target.first_name as targetFirstName, target.last_name AS targetLastName, target.image as targetImage, transactions.amount as amount, transactions.type as type, transactions.info as info, transactions.created_at as created_at, transactions.updated_at as updated_at FROM transactions LEFT JOIN users as user ON transactions.user_id = user.id LEFT JOIN users as target ON transactions.target_id = target.id WHERE (transactions.user_id LIKE '%${user}%') OR (transactions.target_id LIKE '%${user}%') AND`
